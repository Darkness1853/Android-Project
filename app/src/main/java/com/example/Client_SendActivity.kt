...
class Client_SendActivity : AppCompatActivity(), LocationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSend.setOnClickListener {
            if (isConnect && currentLocation != null) {
                ...
                Toast.makeText(this, "Сначала подключитесь к серверу", Toast.LENGTH_SHORT).show()
            } else {
        ...
        Thread {
            ...
            runOnUiThread {
                ...
                if (finalConnected) {
                    ...
                    tvStatus.text = "Статус: Подключено к $ip:$port"
    ...
    private fun getMobileNetworkData(): org.json.JSONObject {
        val networksArray = org.json.JSONArray()
                ...
                if (cellInfos != null) {
                    for (cellInfo in cellInfos) {
                        if (cellInfo is CellInfoLte ) {
                            ...
                            val networkJson = org.json.JSONObject().apply {
                                put("NetworkType", "CellInfoLte")
                                put("CellIdentity", cellIdentity.ci.toString())
                                put("MCC", cellIdentity.mccString ?: "")
                                put("MNC", cellIdentity.mncString ?: "")
                                put("PCI", cellIdentity.pci)
                                put("TAC", cellIdentity.tac)
                                put("Bands", cellIdentity.bands?.joinToString(",") ?: "")

                                val rsrp = cellSignalStrength.dbm
                                put("SignalStrength", "RSRP: $rsrp")
                                put("RSRP", rsrp)
                                put("RSRQ", cellSignalStrength.rsrq)
                                put("RSSI", cellSignalStrength.rssi)
                                put("TimingAdvance", cellSignalStrength.timingAdvance)
        ...
        return org.json.JSONObject().apply {
            put("MobileNetworks", networksArray)
        }
    }
    ...
    private fun sendLocation() {
        ...
        val mobileNetworkData = getMobileNetworkData()

        val json = org.json.JSONObject().apply {
            put("mobile_network_data_list", mobileNetworkData)
            ...
            val locationData = org.json.JSONObject().apply {
                ...
                put("Time", currentTime)
                put("TimeFormatted", timeFormat.format(Date()))
                put("Speed", location.speed)
                put("Bearing", location.bearing)
                put("Accuracy", location.accuracy)
                put("VerticalAccuracy", location.verticalAccuracyMeters)
            }
            put("location_data", locationData)

            put("send_timestamp", currentTime)
            put("send_id", currentSendNumber)
            put("device_time", timeFormat.format(Date()))
            put("device_time_iso", isoFormat.format(Date()))
        }
        ...
        Thread {
            ...
            try {
                ...
                if (reply != null) {
                    ...
                    runOnUiThread {
                        if (isSending) {
                            tvStatus.text = "Статус: Нет ответа"
                        }
                    }
                }
                ...
                runOnUiThread {
                    if (isSending) {
                        tvStatus.text = "Статус: Ошибка ${e.message}"
                    }
                }
            } catch (e: Exception) {
                runOnUiThread {
