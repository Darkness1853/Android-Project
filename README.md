...
1. Как реализовать при нажатии рандомный цвет кнопки?
   ...
   Используя:
    ...
    - setBackgroundColor - для установки цвета и возврата на исходный цвет
      ...
      ```
***Задание по Медиаплееру***
Научиться работать с файлами внутренней (или внешней) памяти смартфона. Исползование класса MediaPlayer для MP3-проигрователя.

- Создать новое Activity для работы с MediaPlayer;
- Функции MediaPlayer : воспроизведение текущего трека, пауза текущего трека, обработка перехода Activity в состояние onPause(), регулировка громкости, SeekBar для отображения текущей длительности трека и его движение, перемотка трека (при помощи SeekBar)
- Возможность вопроизводить музыку из хранилища телефона при помощи Permission
- Реализовать проверку текущего файла на директорию (isDirectory);
